...
app = FastAPI(title="GPT Cyber Content API", version="0.56.0")
...
def health():
    return {
        "status":"ok", "version":"0.56.0", "openai_configured":bool(os.getenv("OPENAI_API_KEY")),
...
def _supporting_promised_counts(post, content):
    arabic_digits=str.maketrans("٠١٢٣٤٥٦٧٨٩","0123456789")
    words={"واحد":1,"واحدة":1,"اثنان":2,"اثنين":2,"ثلاثة":3,"ثلاث":3,"أربعة":4,"أربع":4,"خمسة":5,"خمس":5,"ستة":6,"ست":6,"سبعة":7,"سبع":7,"ثمانية":8,"ثمان":8,"تسعة":9,"تسع":9,"عشرة":10,"عشر":10}
    texts=[post.get("title"),post.get("text"),content.get("hook"),content.get("caption"),content.get("cta")]
    texts += [x.get("headline","")+" "+x.get("body","") for x in content.get("slides",[]) if isinstance(x,dict)]
    combined=" ".join(str(x or "") for x in texts).translate(arabic_digits)
    token=r"(?:\d{1,2}|واحد(?:ة)?|اثنان|اثنين|ثلاث(?:ة)?|أربع(?:ة)?|خمس(?:ة)?|ست(?:ة)?|سبع(?:ة)?|ثمان(?:ية)?|تسع(?:ة)?|عشر(?:ة)?)"
    def find(labels):
        match=re.search(rf"({token})\s+(?:{labels})",combined)
        if not match:return None
        value=match.group(1)
        return int(value) if value.isdigit() else words.get(value)
    return find(r"سؤال(?:اً|ا)?|أسئلة|نقطة|نقاط|بند|بنود|خطوة|خطوات|إجراء|إجراءات|توصية|توصيات"),find(r"محور|محاور|قسم|أقسام")

def _supporting_points_count(pages):
    return sum(len(page.get("bullets") or []) for page in pages)

def _supporting_outline(post, req: SupportingFileGenerateRequest):
    content=post.get("content") or {}; slides=content.get("slides") or []
    promised_points,promised_groups=_supporting_promised_counts(post,content)
    ...
    defaults=[
        ...
        {"headline":"خطوات التطبيق","body":"حوّل الفكرة إلى إجراءات واضحة يمكن متابعتها وقياسها داخل المؤسسة.","bullets":practical},
        {"headline":"قائمة مراجعة عملية","body":"استخدم النقاط التالية عند المراجعة أو اتخاذ القرار.","bullets":[]},
    ...
    for index,default in enumerate(defaults):
        ...
        fallback_pages.append({"headline":str(slide.get("headline") or default["headline"]),"body":f"{slide_body} {default_body}".strip()[:700],"bullets":default["bullets"]})
    fallback={"title":req.title.strip() or post["title"],"subtitle":f"{req.file_type} مبسط للتطبيق العملي","pages":fallback_pages[:5],"sources":source_names[:4],"research_mode":"user_reference" if grounding else "post_content","promised_points":promised_points,"promised_groups":promised_groups}
    if not os.getenv("OPENAI_API_KEY"):
        if promised_points and _supporting_points_count(fallback_pages)!=promised_points: raise HTTPException(503,"يلزم إعداد OpenAI لإنشاء جميع النقاط المعلن عنها دون نقص")
        return fallback
    page_count=promised_groups or 5
    prompt=f'''أنشئ ملفًا داعمًا عربيًا احترافيًا لمنشور LinkedIn، وسيضاف له غلاف منفصل.
...
- أنشئ {page_count} صفحات داخلية بالضبط، كل صفحة: عنوان أقصى 9 كلمات، فقرة 20-45 كلمة.
- العدد المعلن للنقاط أو الأسئلة هو {promised_points or 'غير محدد'}. إذا كان محددًا، يجب أن يكون مجموع bullets في جميع الصفحات مساويًا له حرفيًا، دون دمج نقطتين أو حذف أو تكرار.
- العدد المعلن للمحاور أو الأقسام هو {promised_groups or 'غير محدد'}. إذا كان محددًا، خصص صفحة مستقلة لكل محور.
- اجعل كل bullet قصيرة وواضحة، بحد أقصى 18 كلمة. لا تكتب رقمًا في العنوان أو الفقرة لا يطابق العناصر الظاهرة.
...
- أعد JSON فقط: title, subtitle, pages[{{headline,body,bullets}}], sources[{{name,url}}].'''
    try:
        ...
        if len(pages)!=page_count: raise HTTPException(502,"لم يلتزم النموذج بعدد المحاور المعلن؛ لم يتم إنشاء ملف ناقص")
        if any(not isinstance(page,dict) or not isinstance(page.get("bullets") or [],list) for page in pages):
            raise HTTPException(502,"أعاد النموذج النقاط بتنسيق غير صالح؛ أعد المحاولة")
        ...
        for page in pages:
            ...
            if banned.search(text): raise HTTPException(502,"احتوى الملف على صياغة شخصية غير معتمدة؛ أعد المحاولة")
        if promised_points and _supporting_points_count(pages)!=promised_points:
            raise HTTPException(502,f"أعاد النموذج {_supporting_points_count(pages)} نقطة بدل {promised_points}؛ لم يتم حفظ ملف غير مكتمل")
        generated_points,generated_groups=_supporting_promised_counts(
            {"title":data.get("title"),"text":data.get("subtitle")},
            {"slides":pages},
        )
        if generated_points and generated_points!=_supporting_points_count(pages):
            raise HTTPException(502,"العدد المكتوب داخل الملف لا يطابق عدد النقاط الفعلي؛ لم يتم حفظ الملف")
        if generated_groups and generated_groups!=len(pages):
            raise HTTPException(502,"عدد المحاور المكتوب لا يطابق عدد صفحات المحاور؛ لم يتم حفظ الملف")
        ...
        if research_mode=="official_web":
            ...
            if not web_sources:raise HTTPException(502,"لم يُرجع البحث مصدرًا رسميًا معتمدًا؛ لم يتم إنشاء الملف")
        final_sources=source_names[:4] if research_mode=="user_reference" else web_sources[:4]
        return {"title":str(data.get("title") or fallback["title"])[:500],"subtitle":str(data.get("subtitle") or fallback["subtitle"])[:300],"pages":[{"headline":str(p.get("headline") or "")[:300],"body":str(p.get("body") or "")[:900],"bullets":[str(x)[:220] for x in (p.get("bullets") or [])]} for p in pages],"sources":final_sources,"research_mode":research_mode,"promised_points":promised_points,"promised_groups":promised_groups}
    except HTTPException:
        raise
    except Exception:
        if promised_points: raise HTTPException(502,"تعذر توليد جميع النقاط المعلن عنها؛ لم يتم حفظ ملف ناقص")
        return fallback

def _supporting_pdf_carousel(outline):
    from PIL import Image,ImageDraw,ImageFont
    # 1240 x 1754 pixels at 150 DPI produces an ISO A4 page (595.2 x 841.92 pt).
    width,height=1240,1754; resolution=150; font_path=str(BASE_DIR/"assets"/"fonts"/"Cairo.ttf")
    ...
    def font(size):
        ...
        return fonts[size]
    def wrapped(draw,text,max_width,size):
        ...
        return lines
    def right_lines(draw,lines,x,y,size,leading,color=body):
        for row in lines:
            draw.text((x,y),row,font=font(size),fill=color,anchor="ra",direction="rtl",language="ar");y+=leading
        return y
    def fitted(draw,text,max_width,max_height,start_size,min_size,extra_leading=16):
        for size in range(start_size,min_size-1,-1):
            lines=wrapped(draw,text,max_width,size);leading=size+extra_leading
            if len(lines)*leading<=max_height:return size,leading,lines
        lines=wrapped(draw,text,max_width,min_size);leading=min_size+extra_leading
        if len(lines)*leading>max_height:raise ValueError("النص أطول من المساحة الآمنة في صفحة A4")
        return min_size,leading,lines
    def logo(draw,dark=False):
        color="#FFFFFF" if dark else navy;draw.rounded_rectangle((78,65,170,157),radius=20,outline=cyan,width=5);draw.line((92,111,111,111,123,89,139,132,152,111,165,111),fill=cyan,width=5,joint="curve");draw.text((485,88),"نبض سيبراني",font=font(30),fill=color,anchor="ra",direction="rtl",language="ar");draw.text((210,122),"CYBER PULSE",font=font(18),fill=cyan,anchor="la")
    def footer(draw,page,total):
        draw.rectangle((0,1680,width,height),fill=navy);draw.text((1150,1717),"دليل نبض سيبراني",font=font(17),fill="#FFFFFF",anchor="ra",direction="rtl",language="ar");draw.text((78,1717),f"{page}/{total}",font=font(16),fill="#FFFFFF",anchor="la")
    pages=[];total=len(outline["pages"])+1
    im=Image.new("RGB",(width,height),navy);draw=ImageDraw.Draw(im);draw.ellipse((-380,930,610,1900),fill=navy2)
    for i in range(8):draw.arc((-340+i*38,860+i*28,720+i*48,1750+i*52),15,135,fill="#3E6B64",width=2)
    logo(draw,True);draw.rounded_rectangle((830,225,1150,290),radius=30,fill=cyan);draw.text((990,257),"ملف داعم | دليل عملي",font=font(23),fill=navy,anchor="mm",direction="rtl",language="ar")
    title_size,title_leading,title_lines=fitted(draw,outline["title"],1000,380,78,48,20)
    title_end=right_lines(draw,title_lines,1120,500,title_size,title_leading,"#FFFFFF");draw.rectangle((840,title_end+45,1120,title_end+52),fill=cyan)
    sub_size,sub_leading,sub_lines=fitted(draw,outline.get("subtitle",""),900,260,34,24,16)
    right_lines(draw,sub_lines,1120,title_end+115,sub_size,sub_leading,"#D8EAE7");footer(draw,1,total);pages.append(im)
    for index,page in enumerate(outline["pages"],2):
        im=Image.new("RGB",(width,height),paper);draw=ImageDraw.Draw(im);draw.rectangle((0,0,width,28),fill=navy);draw.rectangle((0,28,width,36),fill=cyan);logo(draw,False)
        draw.rounded_rectangle((78,178,285,232),radius=27,fill=pale);draw.text((181,205),f"الفصل {index-1}",font=font(20),fill=navy,anchor="mm",direction="rtl",language="ar")
        draw.rounded_rectangle((1000,245,1150,395),radius=32,fill=navy);draw.text((1075,320),f"{index-1:02d}",font=font(51),fill=cyan,anchor="mm")
        heading_size,heading_leading,heading_lines=fitted(draw,page.get("headline",""),820,205,55,36,18)
        heading_end=right_lines(draw,heading_lines,930,270,heading_size,heading_leading,ink);draw.rectangle((720,max(465,heading_end+30),930,max(472,heading_end+37)),fill=cyan)
        card_top=max(540,heading_end+95);card_bottom=1605;inner_top=card_top+70;inner_bottom=card_bottom-60
        sources_text=""
        if index==total and outline.get("sources"):sources_text="المراجع: "+" | ".join(outline["sources"])
        source_lines=wrapped(draw,sources_text,990,17) if sources_text else []
        source_height=len(source_lines)*29+(35 if source_lines else 0)
        available=inner_bottom-inner_top-source_height
        bullets=[str(x) for x in (page.get("bullets") or [])]
        chosen=None
        for body_size in range(34,17,-1):
            body_lines=wrapped(draw,page.get("body",""),950,body_size);body_leading=body_size+17
            for bullet_size in range(29,17,-1):
                bullet_leading=bullet_size+13;bullet_rows=[wrapped(draw,x,835,bullet_size) for x in bullets]
                bullet_heights=[max(54,len(rows)*bullet_leading+12) for rows in bullet_rows]
                used=len(body_lines)*body_leading+(38 if bullets else 0)+sum(bullet_heights)+max(0,len(bullets)-1)*12
                if used<=available:chosen=(body_size,body_leading,body_lines,bullet_size,bullet_leading,bullet_rows,bullet_heights);break
            if chosen:break
        if not chosen:raise ValueError("تعذر توزيع النص والنقاط دون تداخل في صفحة A4")
        draw.rounded_rectangle((78,card_top,1162,card_bottom),radius=32,fill="#FFFFFF",outline=line,width=2);draw.rectangle((1120,card_top+50,1128,card_bottom-50),fill=cyan)
        body_size,body_leading,body_lines,bullet_size,bullet_leading,bullet_rows,bullet_heights=chosen
        y=right_lines(draw,body_lines,1075,inner_top,body_size,body_leading,body)
        if bullets:y+=38
        for number,(rows,item_height) in enumerate(zip(bullet_rows,bullet_heights),1):
            draw.ellipse((1030,y-6,1080,y+44),fill=pale);draw.text((1055,y+19),str(number),font=font(18),fill=cyan,anchor="mm")
            right_lines(draw,rows,995,y,bullet_size,bullet_leading,ink);y+=item_height+12
        if source_lines:right_lines(draw,source_lines,1100,inner_bottom-source_height+35,17,29,"#6D817E")
        footer(draw,index,total);pages.append(im)
    buffer=io.BytesIO();pages[0].save(buffer,format="PDF",save_all=True,append_images=pages[1:],resolution=resolution,quality=90,optimize=True);return buffer.getvalue()
...
def linkedin_studio_supporting_file_generate(post_id: str, req: SupportingFileGenerateRequest):
    ...
    return {"generated":True,"id":asset_id,"name":name,"mime_type":"application/pdf","size":len(pdf),"title":outline["title"],"pages":len(outline["pages"])+1,"points":_supporting_points_count(outline["pages"]),"page_size":"A4","research_mode":outline.get("research_mode"),"sources":outline.get("sources",[])}
