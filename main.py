...
app = FastAPI(title="GPT Cyber Content API", version="0.3.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def demo_payload(req: ContentRequest):
    ...
    return {
        ...
        "recommendations": ["حدد مالكًا واضحًا للمخاطر والضوابط.", "اربط المتطلبات التنظيمية بضوابط قابلة للقياس.", "راجع الأدلة والاستثناءات بشكل دوري."],
        ...
        "slides": [{"number": i + 1, "headline": req.topic if i == 0 else f"النقطة {i + 1}", "body": "نص تجريبي للمعاينة."} for i in range(n)],
        "sources": [],
    }
...
@app.get("/health")
def health():
    return {"status": "ok", "openai_configured": bool(os.getenv("OPENAI_API_KEY")), "version": "0.3.0", "web_ui": INDEX_FILE.exists()}
...
{{"title":"...","hook":"...","caption":"...","recommendations":["..."],"cta":"...","keywords":["..."],"hashtags":["#..."],"slides":[{{"number":1,"headline":"...","body":"..."}}],"sources":[{{"name":"...","url":"...","why_relevant":"..."}}]}}
"""
...
def generate_image(req: ImageRequest):
    ...
    prompt = (
        ...
        f"Slide headline concept: {req.title}. Supporting meaning: {req.body}. {req.visual_direction}. "
        "Use a sophisticated editorial infographic/cybersecurity visual language with a concept directly related to this specific slide rather than generic hacker imagery. "
        "Use UAE-appropriate professional institutional visual cues when people or environments appear. Do not include readable text, letters, logos, watermarks, UI screenshots, or brand marks. "
