...
(() => {
  ...
  function showReview(jobId, job) {
    ...
    byId("vaApprove").onclick = async () => {
      ...
      try {
        const response = await fetch(`/api/visual-alert/approve/${encodeURIComponent(jobId)}`, {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({asset_order:assets.map((asset)=>asset.token),motion_overlays:byId("vaMotionOverlays").checked})});
